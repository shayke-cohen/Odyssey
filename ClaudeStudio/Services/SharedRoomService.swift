...
final class SharedRoomService: ObservableObject {
    ...
    func currentUserIdentity() -> UserIdentity {
        ...
        let displayName = {
            ...
            return Host.current().localizedName ?? "ClaudeStudio User"
        }()
    ...
    func createInvite(
        ...
        let deepLink = "claudestudio://room/join?roomId=\(roomId)&inviteId=\(inviteId)&token=\(inviteToken)"
    ...
    private static func makeLocalRecordStore() -> SharedRoomLocalRecordStore? {
        ...
        guard environment["CLAUDESTUDIO_SHARED_ROOM_BACKEND"] == "local-test" else {
            return nil
        }

        let storeURL: URL
        if let explicitPath = environment["CLAUDESTUDIO_SHARED_ROOM_STORE_PATH"], !explicitPath.isEmpty {
            storeURL = URL(fileURLWithPath: explicitPath)
        } else {
            storeURL = URL(fileURLWithPath: NSHomeDirectory())
                .appendingPathComponent(".claudestudio/shared-room-test/store.plist")
        }

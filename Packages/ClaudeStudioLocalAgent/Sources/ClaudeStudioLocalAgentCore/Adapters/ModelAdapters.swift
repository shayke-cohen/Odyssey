    ...
    func probe() async -> ProviderProbeResult {
        ...
        guard let command else {
            return ProviderProbeResult(
                provider: provider,
                available: false,
                reason: "MLX tool runner not found. Set CLAUDESTUDIO_MLX_RUNNER or install llm-tool from mlx-swift-examples.",
                supportsTools: true,
                supportsTranscriptResume: true
            )
        }
...
private struct MLXCommandGenerator: LocalAgentTextGenerating {
    ...
    func generate(prompt: String, config: LocalAgentConfig) async throws -> String {
        guard let command else {
            throw NSError(domain: "ClaudeStudioLocalAgent.MLX", code: 1, userInfo: [

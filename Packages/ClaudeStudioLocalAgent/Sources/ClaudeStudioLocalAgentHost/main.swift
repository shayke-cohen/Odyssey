import ClaudeStudioLocalAgentCore
import Foundation
...
actor HostService {
    ...
    func handle(method: String, params: Any?) async throws -> Any {
        ...
        case LocalAgentHostMethod.initialize.rawValue:
            return ["name": "ClaudeStudioLocalAgentHost", "version": "0.2.0"]
        ...
        default:
            throw NSError(domain: "ClaudeStudioLocalAgentHost", code: -32601, userInfo: [
        ...
        do {
            ...
            if let numericID = id as? Int {
                if let error = root["error"] as? [String: Any] {
                    await broker.fail(
                        id: numericID,
                        error: NSError(domain: "ClaudeStudioLocalAgentHost", code: error["code"] as? Int ?? -32000, userInfo: [
...
private func chatCommand(arguments: [String]) async throws {
    ...
    print("ClaudeStudioLocalAgentHost interactive chat")
    ...
    case "/help":
        print(
            ...
            Examples:
              list files here
              read AGENTS.md
              search for "LocalProviderSupport" in ClaudeStudio
...
Task {
    ...
    do {
        ...
        fputs("ClaudeStudioLocalAgentHost error: \(error.localizedDescription)\n", stderr)
        exit(1)
    }
}
